...
const Header: FC = () => {
    ...
    useEffect(() => {
        if (isMounted.current) {
            const cart = JSON.stringify(itemsCart);
    ...
    const totalCartCount = itemsCart.reduce((count, currentItem) => {
        ...
        <div className={styles.container}>
            <Link to='/'>
                <div className={styles.headerLogo}>
                    ...
                    <div>
                        <h1>Pizza - и точка</h1>
                        <p>уже 10 тысяч лет на рынке</p>
                    </div>
                </div>
            </Link>
            <div className={styles.headerColumn}>
                {location.pathname !== '/cart' && location.pathname !== '/popular' &&
                    <Search />}
                {location.pathname !== '/cart' && <div className={styles.headerCart}>
                    <div
                        className={clsx(styles.menu, isModal && styles.active)}
