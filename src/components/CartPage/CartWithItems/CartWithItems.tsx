...
const CartWithItems: FC<TProps> = ({ itemsCart, totalPrice }) => {
    const dispatch = useDispatch<AppDispatch>();

    const totalCount = itemsCart.reduce((count, currentItem) => {
    ...
    return <div className={styles.container}>
        <div className={styles.top}>
            ...
            <div onClick={onCartClear} className={styles.clear}>
        ...
        <div className={styles.items}>
            {itemsCart.map((i, index) => <CartItem key={index} item={i.item} count={i.count} />)}
        </div>
        <div className={styles.bottom}>
            <div className={styles.items}>
                <span>
                    Всего продукции: <b>{totalCount}  шт.</b>
                </span>
                <span>
            ...
            <div className={styles.buttons}>
                <Link to='/' className={clsx(styles.button, styles.buttonBack)}>
                    <svg width="8" height="14" viewBox="0 0 8 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M7 13L1 6.93015L6.86175 1" stroke="#D3D3D3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                    <span>Вернуться назад</span>
                </Link>
                <div className={clsx(styles.button, styles.payBtn)}>
